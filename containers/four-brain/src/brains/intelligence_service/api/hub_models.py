"""
Orchestrator Hub API Models (formerly K2 Vector Bridge)
Pydantic models for Intelligence Service endpoints.

Note: This file replaces k2_models.py. A compatibility shim remains to avoid breaking imports.
"""
...
class StrategyResponse(BaseModel):
    """Response model for strategy selection"""
...
class HealthResponse(BaseModel):
    """Health check response for Intelligence Service bridge"""
    service: str = Field(default="orchestrator_hub_bridge", description="Service name")
    status: str = Field(..., description="Service status")
    api_configured: bool = Field(..., description="Whether upstream API is configured")
...
class VectorResponse(BaseModel):
    ...
    strategy: StrategyResponse = Field(..., description="Strategy selection result")
