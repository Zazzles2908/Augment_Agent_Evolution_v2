Param(
  ...
  [string]$ModelName = "qwen3_embedding_trt"
)

$payload = @{ 
  inputs = @(
    @{ name = "input_ids"; datatype = "INT32"; shape = @(1,8); data = @(101,2009,2001,1037,2204,2154,102,0) },
    @{ name = "attention_mask"; datatype = "INT32"; shape = @(1,8); data = @(1,1,1,1,1,1,1,0) }
  );
  outputs = @(@{ name = "embedding"; binary_data = $false })
}

Write-Host "== TRT INT32 minimal inference ($ModelName) =="
# Ensure model loaded
