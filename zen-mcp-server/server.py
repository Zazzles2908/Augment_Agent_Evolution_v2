...
def configure_providers():
    ...
    api_keys_to_check = ["OPENAI_API_KEY", "OPENROUTER_API_KEY", "GEMINI_API_KEY", "XAI_API_KEY", "KIMI_API_KEY", "GLM_API_KEY", "CUSTOM_API_URL"]
    ...
    if gemini_key and gemini_key != "your_gemini_api_key_here":
        try:
            from providers.gemini import GeminiModelProvider as _Gem
            GeminiModelProvider = _Gem  # type: ignore
            valid_providers.append("Gemini")
            has_native_apis = True
            logger.info("Gemini API key found - Gemini models available")
        except Exception:
            logger.warning("Gemini provider import failed; continuing without Gemini")
    ...
    if openai_key and openai_key != "your_openai_api_key_here":
        try:
            from providers.openai_provider import OpenAIModelProvider as _OpenAI
            OpenAIModelProvider = _OpenAI  # type: ignore
            valid_providers.append("OpenAI")
            has_native_apis = True
            logger.info("OpenAI API key found - OpenAI models available")
        except Exception:
            logger.warning("OpenAI provider import failed; continuing without OpenAI")
    else:
        if not openai_key:
    ...
    if dial_key and dial_key != "your_dial_api_key_here":
        try:
            from providers.dial import DIALModelProvider as _Dial
            DIALModelProvider = _Dial  # type: ignore
            valid_providers.append("DIAL")
            has_native_apis = True
            logger.info("DIAL API key found - DIAL models available")
        except Exception:
            logger.warning("DIAL provider import failed; continuing without DIAL")
