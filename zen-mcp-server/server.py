...
# EARLY DIAGNOSTIC: print to stderr so failed imports are visible to callers (e.g., Auggie CLI)
print("[zen-mcp] bootstrap starting (pid=%s, py=%s)" % (os.getpid(), sys.executable), file=sys.stderr)
...
try:
    ...
    try:
        ...
        msg = f"[zen-mcp] dotenv load failed: {dotenv_err}"
...
try:
    ...
    print(f"[zen-mcp] MCP import failed: {mcp_import_err}", file=sys.stderr)
...
use_json_logs = os.getenv("LOG_FORMAT", "").lower() in {"json", "structured", "jsonl"}
...
def configure_providers():
    ...
    if kimi_key and kimi_key != "your_kimi_api_key_here" and "KIMI" not in disabled_providers:
        try:
            from providers.moonshot.provider import KimiModelProvider as _Kimi
    ...
    if glm_key and glm_key != "your_glm_api_key_here" and "GLM" not in disabled_providers:
        try:
            from providers.zhipu.provider import GLMModelProvider as _GLM
...
async def handle_call_tool(name: str, arguments: dict[str, Any]) -> list[TextContent]:
    ...
    if name in TOOLS:
        ...
        try:
            import os, datetime
            if os.getenv("INJECT_CURRENT_DATE", "true").lower() == "true":
...
async def main():
    ...
    if os.getenv("ENABLE_CONFIG_VALIDATOR", "true").lower() == "true":
        try:
            from utils.config_bootstrap import ServerConfig
            _cfg = ServerConfig.load_and_validate()
        except Exception as _e:
    ...
    async with stdio_server() as (read_stream, write_stream):
        # Emit a clear handshake breadcrumb on stderr for CLI debugging
        try:
            print("[zen-mcp] stdio_server started; awaiting MCP handshake...", file=sys.stderr)
        except Exception:
            pass
        await server.run(
            read_stream,
