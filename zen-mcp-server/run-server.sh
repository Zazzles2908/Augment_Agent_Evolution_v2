...
cleanup_docker() {
    ...
    # Remove images
    local images=("-mcp-server:latest" "zen-mcp-server:latest")
...
check__cli_integration() {
    ...
    if grep -q '"zen"' "$_config" 2>/dev/null; then
        # Already configured
        return 0
    fi
    ...
    echo ""
    read -p "Configure Zen for  CLI? (Y/n): " -n 1 -r
    echo ""
    if [[ $REPLY =~ ^[Nn]$ ]]; then
...
" && mv "$temp_file" "$_config"
    ...
    if [[ $? -eq 0 ]]; then
        ...
        echo "  Config: $_config"
        echo "  Restart  CLI to use Zen MCP Server"
    else
